...
## TABELLA 1 - OBESITY GROUPS ##
tab_obesity_full <- bind_rows(
  cohort %>%
    summarise(
      Group = "Overall",
      N = n(),
      BMI_n = sum(obesity_bmi == 1, na.rm = TRUE),
      Lancet_n = sum(obesity_lancet == 1, na.rm = TRUE),
      Preclinical_n = sum(obesity_preclinical == 1, na.rm = TRUE),
      Clinical_n = sum(obesity_clinical == 1, na.rm = TRUE)
    ),
  cohort %>%
    filter(sex == "male") %>%
    summarise(
  ...
  mutate(
    `BMI-defined obesity, n (%)` = paste0(
      BMI_n, " (", round(100 * BMI_n / N, 1), "%)"
    ),
    `Lancet-defined obesity, n (%)` = paste0(
      Lancet_n, " (", round(100 * Lancet_n / N, 1), "%)"
    ),
    `Preclinical obesity, n (%)` = paste0(
      Preclinical_n, " (", round(100 * Preclinical_n / N, 1), "%)"
    ),
    `Clinical obesity, n (%)` = paste0(
      Clinical_n, " (", round(100 * Clinical_n / N, 1), "%)"
    ),
    `Preclinical among Lancet obesity, n (%)` = ifelse(
      Lancet_n > 0,
      paste0(Preclinical_n, " (", round(100 * Preclinical_n / Lancet_n, 1), "%)"),
      "0 (0.0%)"
    ),
    `Clinical among Lancet obesity, n (%)` = ifelse(
      Lancet_n > 0,
      paste0(Clinical_n, " (", round(100 * Clinical_n / Lancet_n, 1), "%)"),
      "0 (0.0%)"
    )
  ) %>%
  select(
    Group,
    N,
    `BMI-defined obesity, n (%)`,
    `Lancet-defined obesity, n (%)`,
    `Preclinical obesity, n (%)`,
    `Clinical obesity, n (%)`,
    `Preclinical among Lancet obesity, n (%)`,
    `Clinical among Lancet obesity, n (%)`
  )
...
write.table(
  tab_obesity_full,
  file = "C:/Users/nicol/Desktop/Table1_BMI_vs_LancetDefinition_full.csv",
  sep = ";",
  row.names = FALSE,
  quote = FALSE,
  fileEncoding = "UTF-8"
